...
1. **Curve forecast (primary task).** Given history at issue time \(t_0\), emit JSON with `hs_forecast_m` of length 24 (hourly leads \(+1\ldots+24\) h).
2. **Exploratory companion.** Emit `wave_regime` and `predictability_24h` labels with free-text notes (separate LoRA adapter). This task is treated as an exploratory extension of the structured-output interface rather than as a co-equal contribution.
...
### 5.2 Exploratory companion task (n = 24)
...
10. Jin, M., et al., Time-LLM: Time series forecasting by reprogramming large language models. ICLR 2024. https://doi.org/10.48550/arXiv.2310.01728  
